...
final class Languages {
    ...
    public static function normalize( array $tags ) {
        ...
        foreach ( $tags as $tag ) {
            ...
            if ( $code === '' || ! preg_match( '/^[a-z]{2,3}$/', $code ) ) continue;
            if ( ! in_array( $code, $out, true ) ) $out[] = $code;
        }
        return $out;
    }
    ...
    private static function translatepress() {
        ...
        $codes = array();
        if ( ! empty( $settings['default-language'] ) ) $codes[] = (string) $settings['default-language'];
