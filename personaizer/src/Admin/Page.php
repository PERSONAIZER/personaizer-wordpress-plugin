...
final class Page {
    ...
    public static function settings() {
        register_setting( 'personaizer', Options::IDENTIFY_USERS, array( 'type' => 'string', 'sanitize_callback' => static function ( $v ) { return $v === '1' ? '1' : ''; } ) );
    }
    ...
    public static function model() {
        ...
        foreach ( Streams::all() as $key => $meta ) {
            ...
            $streams[ $key ] = array(
                ...
                'queue'    => $counts[ $key ] ?? array( 'queued' => 0, 'deferred' => 0, 'failed' => 0 ),
        ...
        return array(
            ...
            'state'       => $state,
            'streams'     => $streams,
            'backfill'    => $connected ? Backfill::progress() : array( 'running' => false, 'enqueued' => 0 ),
            'reconcile'   => $connected ? Reconcile::progress() : array( 'running' => false, 'stream' => null ),
    ...
    private static function is_busy() {
        if ( ! Options::is_connected() ) return false;
        $state = State::get();
