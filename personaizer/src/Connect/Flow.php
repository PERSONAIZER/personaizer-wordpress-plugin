...
final class Flow {
    ...
    public static function connect() {
        ...
        set_transient( 'personaizer_connect_' . $state, array( 'connect_id' => $started['connect_id'], 'verifier' => $verifier ), self::ATTEMPT_TTL );
        wp_safe_redirect( add_query_arg( array( 'c' => $started['connect_id'], 'state' => $state ), rtrim( PERSONAIZER_APP_URL, '/' ) . '/connect' ) );
        exit;
    }
    ...
    public static function callback() {
        ...
        if ( $state !== '' ) delete_transient( 'personaizer_connect_' . $state );

        if ( $error !== '' ) self::back( array( 'pz_error' => 'The connection was not approved.' ) );
        if ( ! is_array( $attempt ) || $code === '' ) self::back( array( 'pz_error' => 'The connection attempt expired — try again.' ) );
