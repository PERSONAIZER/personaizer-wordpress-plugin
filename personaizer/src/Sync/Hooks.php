...
final class Hooks {
    ...
    public static function on_post_saved( $post_id, $post, $update = null, $post_before = null ) {
        ...
        if ( $post->post_type === 'product' ) return;   // WooCommerce's own hooks own products
    ...
    public static function on_post_removed( $post_id ) {
        ...
        if ( ! $post instanceof WP_Post || ! Options::is_connected() ) return;
        $stream = Streams::for_post_type( $post->post_type );
        if ( $stream === null ) return;
    ...
    public static function on_product_object( $product ) {
        if ( is_object( $product ) && method_exists( $product, 'get_id' ) ) self::on_product_changed( $product->get_id() );
    }
    ...
    public static function on_variation_object( $variation ) {
        if ( is_object( $variation ) && method_exists( $variation, 'get_parent_id' ) ) self::on_product_changed( $variation->get_parent_id() );
    }
}
