...
final class State {
    ...
    public static function get( $force = false ) {
        if ( ! Options::is_connected() ) return null;
        if ( ! $force ) {
    ...
    public static function enabled_streams() {
        $state = self::get();
        if ( $state === null || $state['status'] !== 'active' ) return array();
        return array_filter( $state['streams'], static function ( $s ) { return ! empty( $s['enabled'] ); } );
    }
