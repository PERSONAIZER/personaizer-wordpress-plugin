...
final class Backfill {
    ...
    public static function start() {
        ...
        update_option( Options::BACKFILL, array(
    ...
    public static function run() {
        ...
        if ( ! is_array( $state ) || ! empty( $state['finished_at'] ) ) return;
    ...
    public static function progress() {
        $state = get_option( Options::BACKFILL );
        if ( ! is_array( $state ) ) return array( 'running' => false, 'enqueued' => 0 );
        return array( 'running' => empty( $state['finished_at'] ), 'enqueued' => (int) $state['enqueued'] );
    }
