...
final class Reconcile {
    ...
    public static function start() {
        ...
        foreach ( State::enabled_streams() as $key => $_ ) {
            if ( self::can_enumerate( $key ) ) $streams[] = $key;
        }
        update_option( Options::RECONCILE, array(
            'streams' => $streams, 'stream' => null, 'offset' => 0, 'items' => array(),
            'started_at' => time(), 'finished_at' => null,
        ), false );
        self::arm( 0 );
    }
    ...
    public static function run() {
        ...
        while ( microtime( true ) < $deadline ) {
            ...
            if ( count( $slice ) < self::SLICE ) {
                ...
                self::record( $walk['stream'], array( 'error' => 'The stream is larger than a single list can carry.', 'at' => time() ) );
                $walk['stream'] = null;
                $walk['items']  = array();
            }
    ...
    private static function slice( $stream, $offset ) {
        $all = Streams::all();
        if ( ! isset( $all[ $stream ] ) ) return array();
        $items = array();
        if ( $stream === 'products' ) {
            $products = wc_get_products( array( 'status' => 'publish', 'limit' => self::SLICE, 'offset' => $offset, 'orderby' => 'ID', 'order' => 'ASC' ) );
            foreach ( $products as $product ) {
                if ( ! ProductPayload::is_syncable( $product ) ) continue;
                $record  = ProductPayload::build( $product );
                $items[] = array( 'id' => $record['id'], 'fingerprint' => $record['fingerprint'] );
            }
            return $items;
        }
        $posts = get_posts( array(
            'post_type' => $all[ $stream ]['post_type'], 'post_status' => 'publish', 'posts_per_page' => self::SLICE,
            'offset' => $offset, 'orderby' => 'ID', 'order' => 'ASC', 'no_found_rows' => true, 'suppress_filters' => true,
        ) );
        foreach ( $posts as $post ) {
            $record = PostPayload::build( $post );
            if ( $record !== null ) $items[] = array( 'id' => $record['id'], 'fingerprint' => $record['fingerprint'] );
        }
        return $items;
    }
    ...
    private static function send( $stream, array $items ) {
        ...
        if ( is_wp_error( $result ) ) {
            if ( Client::is_closed( $result ) ) State::forget();
            self::record( $stream, array( 'error' => $result->get_error_message(), 'at' => time() ) );
            return;
        }
        $rows = array();
        foreach ( array_merge( $result['missing'], $result['stale'] ) as $external_id ) {
            ...
            if ( $post_id ) $rows[] = array( 'external_id' => $external_id, 'post_id' => $post_id );
        }
        if ( $rows ) Outbox::enqueue_upserts( $stream, $rows );
        self::record( $stream, array(
            'generation' => $result['generation'], 'listed' => count( $items ),
            'missing' => count( $result['missing'] ), 'stale' => count( $result['stale'] ),
            'orphans_deleted' => $result['orphans_deleted'], 'orphans_held' => $result['orphans_held'], 'busy' => $result['busy'],
            'at' => time(),
        ) );
    }
    ...
    private static function can_enumerate( $stream ) {
        $all = Streams::all();
        if ( ! isset( $all[ $stream ] ) ) return false;
        if ( $stream === 'products' ) return Streams::has_woocommerce() && function_exists( 'wc_get_products' );
    ...
    public static function progress() {
        ...
        return array( 'running' => $running, 'stream' => $running ? $walk['stream'] : null );
    }
