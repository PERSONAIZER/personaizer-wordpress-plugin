...
final class Worker {
    ...
    public static function drain( $budget_seconds ) {
        if ( ! Options::is_connected() ) return 0;
        $deadline = microtime( true ) + $budget_seconds;
        $landed   = 0;

        $on = State::enabled_streams();
        foreach ( Outbox::streams_with_work() as $stream ) {
            if ( ! isset( $on[ $stream ] ) ) continue;   // off, or never switched on: rows wait for the owner
    ...
    private static function push( $stream, $type, array $rows ) {
        ...
        if ( is_wp_error( $result ) ) {
            ...
            $attempts = max( array_map( static function ( $r ) { return (int) $r->attempts; }, $rows ) );
        ...
        $done = array();
        foreach ( $result['written'] as $id ) if ( isset( $row_by_id[ $id ] ) ) $done[] = $row_by_id[ $id ]->id;
        if ( ! $result['deletes_busy'] ) {
            foreach ( $deletes as $id ) if ( isset( $row_by_id[ $id ] ) ) $done[] = $row_by_id[ $id ]->id;
        }
        Outbox::ack( $done );

        $deferred = array();
        foreach ( $result['deferred'] as $id ) if ( isset( $row_by_id[ $id ] ) ) $deferred[] = $row_by_id[ $id ]->id;
        Outbox::defer( $deferred );

        foreach ( $result['rejected'] as $id => $why ) {
            if ( isset( $row_by_id[ $id ] ) ) Outbox::fail( array( $row_by_id[ $id ]->id ), trim( $why['code'] . ' ' . $why['message'] ) );
        }

        if ( count( $result['written'] ) > 0 ) Options::clear_error();
    ...
    private static function record( $type, $row ) {
        if ( $type === Streams::CATALOG ) {
            ...
            if ( ! $product || ! $product->exists() || ! ProductPayload::is_syncable( $product ) ) return null;
