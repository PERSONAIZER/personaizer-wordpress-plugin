...
final class ProductPayload {
    ...
    private static function category_paths( WC_Product $product ) {
        ...
        if ( ! is_array( $terms ) || empty( $terms ) ) return array( array( 'products' ) );

        $paths = array(); $by_key = array(); $term_ids = array();
        foreach ( $terms as $t ) {
            $chain = array();
        ...
        foreach ( $paths as $i => $chain ) {
            ...
            foreach ( $term_ids as $j => $other_id ) {
                if ( $i !== $j && in_array( $term_ids[ $i ], get_ancestors( $other_id, 'product_cat' ), true ) ) { $covered = true; break; }
            }
    ...
    private static function category_names( array $paths ) {
        $names = array();
        foreach ( $paths as $chain ) foreach ( $chain as $name ) $names[ $name ] = true;
    ...
    private static function attributes( WC_Product $product ) {
        ...
        foreach ( $product->get_attributes() as $name => $attribute ) {
            ...
            if ( $key !== '' && ! isset( $attrs[ $key ] ) && ! empty( $options ) ) $attrs[ $key ] = $options;
        }
        return $attrs;
    }
    ...
    private static function variants( WC_Product $product ) {
        ...
        foreach ( $product->get_children() as $vid ) {
            ...
            foreach ( self::variation_facets( $variation ) as $key => $value ) $variant[ $key ] = $value;
            $variants[] = $variant;
        }
        return $variants;
    }
    ...
    private static function simple_variant( WC_Product $product ) {
        ...
        foreach ( self::taxonomy_facets( $product ) as $key => $value ) $variant[ $key ] = $value;
        return $variant;
    }
    ...
    private static function commerce( WC_Product $product ) {
        ...
        if ( $price !== null ) {
            ...
            if ( $product->is_on_sale() && $regular !== null && $regular > $price ) $variant['original_price'] = $regular;
        }
    ...
    private static function images( WC_Product $product ) {
        ...
        if ( $primary_id ) {
            ...
            if ( $url ) { $images[] = array( 'url' => $url, 'description' => '', 'is_primary' => true, 'source' => 'client' ); $seen[ $url ] = true; }
        }
        foreach ( (array) $product->get_gallery_image_ids() as $gid ) {
            ...
            if ( $url && ! isset( $seen[ $url ] ) ) { $seen[ $url ] = true; $images[] = array( 'url' => $url, 'description' => '', 'is_primary' => false, 'source' => 'client' ); }
        }
        return $images;
    }
