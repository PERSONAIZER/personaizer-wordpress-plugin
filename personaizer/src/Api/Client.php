...
final class Client {
    ...
    public static function token( $code, $code_verifier, $redirect_uri ) {
        $body   = array( 'code' => $code, 'code_verifier' => $code_verifier, 'redirect_uri' => $redirect_uri );
    ...
    public static function sync( array $inventory ) {
        $body   = array( 'site_name' => (string) get_bloginfo( 'name' ), 'inventory' => array_values( $inventory ) );
    ...
    public static function items( $stream, array $upserts, array $deletes ) {
        $body   = array( 'upserts' => array_values( $upserts ), 'deletes' => array_values( $deletes ) );
        $result = self::call( 'PUT', '/v1/integration/streams/' . rawurlencode( $stream ) . '/items', $body, self::auth(), 60 );
        if ( ! is_wp_error( $result ) ) update_option( Options::LAST_PUSH, time(), false );
    ...
    public static function reconcile( $stream, $generation, array $items ) {
        $body   = array( 'generation' => (int) $generation, 'items' => array_values( $items ) );
    ...
    public static function disconnect() {
        if ( ! Options::is_connected() ) return true;
    ...
    private static function call( $method, $path, $body, array $headers, $timeout ) {
        if ( isset( $headers['X-Api-Key'] ) && $headers['X-Api-Key'] === '' ) {
            return new WP_Error( 'personaizer_not_connected', 'This site is not connected to PERSONAIZER.', array( 'status' => 0, 'code' => '' ) );
        }
        $args = array(
            'method'  => $method,
            'timeout' => $timeout,
        ...
        if ( is_wp_error( $response ) ) {
            $response->add_data( array( 'status' => 0, 'code' => '' ) );
        ...
        if ( $code !== Contracts::CLOSED ) Options::record_error( $message, $code );
        return new WP_Error( 'personaizer_http_' . $status, $message, array( 'status' => $status, 'code' => $code ) );
    }
